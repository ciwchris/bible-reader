...
This project uses TypeScript, Svelte 5 runes, and tab indentation. Follow the existing style: `PascalCase.svelte` for components, lowercase filenames for stores and server modules such as `lists.ts` or `auth.ts`, and named exports for shared store actions. Prefer small, focused modules and keep shared types in `src/lib/types.ts` instead of duplicating shapes across client and server.

## Testing Guidelines
This repo has automated unit and end-to-end coverage. After implementing a user-facing feature or other cross-cutting change, run the full verification suite: `npm run check`, `npm test`, and `npm run test:e2e`. For smaller localized fixes, run `npm run check` plus the relevant automated tests for the area you touched: `npm test` for unit and component coverage, and `npm run test:e2e` for user-flow changes, routing, auth, sync, or other browser-driven behavior. If your change could affect both layers, run both suites. If you do not run part of the expected verification, state that explicitly in your final response. Keep a quick manual pass for the primary shopping-list flow when the UI or realtime behavior changes. Place new unit tests near the feature or under `src/**/*.test.ts`, and keep names aligned with the module under test. Keep Playwright specs under `e2e/` with scenario-focused filenames.
